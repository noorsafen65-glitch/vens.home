...
export default function App() {
  ...
  // ---------------------------------------------------------
  // هەڵگرتن و پاراستنی 100%ی هەمیشەیی داتاکان لە ناو مۆبایل/کۆمپیوتەر
  // ---------------------------------------------------------
  const saveTransactions = (data) => {
    setAllTransactions(data);
    localStorage.setItem('vh_transactions', JSON.stringify(data));
    try {
      fetch('/api/transactions', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) }).catch(() => {});
    } catch(e) {}
  };

  const saveDebts = (data) => {
    setAllDebts(data);
    localStorage.setItem('vh_debts', JSON.stringify(data));
    try {
      fetch('/api/debts', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) }).catch(() => {});
    } catch(e) {}
  };

  const saveSavings = (data) => {
    setAllSavings(data);
    localStorage.setItem('vh_savings', JSON.stringify(data));
    try {
      fetch('/api/savings', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) }).catch(() => {});
    } catch(e) {}
  };

  // خوێندنەوەی سەرەتایی لە داتابەیس
  useEffect(() => {
    const initData = async () => {
      // سەرەتا لە بیرگەی ئامێرەکە دەخوێنێتەوە
      const localTx = localStorage.getItem('vh_transactions');
      const localSav = localStorage.getItem('vh_savings');
      const localDebts = localStorage.getItem('vh_debts');

      if (localTx) setAllTransactions(JSON.parse(localTx));
      if (localSav) setAllSavings(JSON.parse(localSav));
      if (localDebts) setAllDebts(JSON.parse(localDebts));

      // هەوڵدەدات لە سێرڤەریش بیهێنێت ئەگەر سێرڤەر هەبوو
      try {
        const res = await fetch('/api/data');
        if (res.ok) {
          const serverData = await res.json();
          if (serverData.transactions && serverData.transactions.length > 0) {
            setAllTransactions(serverData.transactions);
            localStorage.setItem('vh_transactions', JSON.stringify(serverData.transactions));
          }
          if (serverData.savings && serverData.savings.length > 0) {
            setAllSavings(serverData.savings);
            localStorage.setItem('vh_savings', JSON.stringify(serverData.savings));
          }
          if (serverData.debts && serverData.debts.length > 0) {
            setAllDebts(serverData.debts);
            localStorage.setItem('vh_debts', JSON.stringify(serverData.debts));
          }
        }
      } catch (err) {
        // لەسەر Vercel دەچێتە لێرە و بە شێوەیەکی سەلامەت لە بیرگەی ناوخۆ کار دەکات
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleAddTransaction = (e) => {
    ...
    if (editingId) {
      const updated = allTransactions.map(t => t.id === editingId ? { ...transactionData, id: editingId } : t);
      saveTransactions(updated);
      setEditingId(null);
      showToast('✅ مامەڵەکە بە سەرکەوتوویی نوێکرایەوە');
    } else {
      const newId = 'tx_' + Date.now();
      const updated = [{ ...transactionData, id: newId }, ...allTransactions];
      saveTransactions(updated);

      // زیادکردن بۆ قەرزەکان ئەگەر پارەی مابێت
      if (remaining > 0) {
        const debtData = {
          id: 'debt_' + Date.now(),
          type: type === 'income' ? 'owes_me' : 'i_owe',
          name: personName,
          jobType: jobType,
          totalAmount: parsedTotal,
          paidAmount: parsedPaid,
          amount: remaining,
          currency: currency,
          note: `ماوەی وەسڵ: ${jobType}`,
          workspace: formWorkspace,
          date: dateStr + ' - ' + timeStr,
          payments: initialPayments,
          createdAt: Date.now() + 1
        };
        saveDebts([debtData, ...allDebts]);
      }

      showToast('✅ مامەڵەکە بە سەرکەوتوویی پاشەکەوت کرا');
    }
  ...
  const handleAddInstallment = (e) => {
    ...
    const curRemaining = parseFloat(installmentModalItem.amount ?? installmentModalItem.remainingAmount ?? 0);
    const newRemaining = Math.max(0, curRemaining - paymentVal);
    const newPaid = (installmentModalItem.paidAmount || 0) + paymentVal;
    ...
    if (installmentModalItem.isDebt) {
      const updated = allDebts.map(d => {
        if (d.id === installmentModalItem.id) {
          return { ...d, amount: newRemaining, paidAmount: newPaid, payments: [...(d.payments || []), newPaymentEntry] };
        }
        return d;
      });
      saveDebts(updated);
      ...
      const updated = allTransactions.map(t => {
        if (t.id === installmentModalItem.id) {
          return { ...t, remainingAmount: newRemaining, paidAmount: newPaid, payments: [...(t.payments || []), newPaymentEntry] };
        }
        return t;
      });
      saveTransactions(updated);
    ...
    showToast('✅ قیستەکە تۆمارکرا و لە بڕی ماوە کەمکرایەوە');
  ...
  const handleAddSavings = (e) => {
    ...
    saveSavings([savingsData, ...allSavings]);
  ...
  const handleDeleteSavings = (id) => {
    ...
    saveSavings(allSavings.filter(t => t.id !== id));
  ...
  const handleAddDebt = (e) => {
    ...
    if (editingDebtId) {
      const updated = allDebts.map(d => d.id === editingDebtId ? { ...debtData, id: editingDebtId } : d);
      saveDebts(updated);
      setEditingDebtId(null);
      showToast('✅ قەرزەکە نوێکرایەوە');
    } else {
      const newId = 'debt_' + Date.now();
      saveDebts([{ ...debtData, id: newId }, ...allDebts]);
      showToast('✅ قەرزەکە بە سەرکەوتوویی پاشەکەوت کرا');
    }
  ...
  const handleDeleteDebt = (id) => {
    if (!confirm('ئایا دڵنیایت لە سڕینەوە؟')) return;
    saveDebts(allDebts.filter(t => t.id !== id));
  ...
  const handleDownloadBackup = () => {
    const fullBackup = { transactions: allTransactions, savings: allSavings, debts: allDebts, date: new Date().toISOString() };
    const blob = new Blob([JSON.stringify(fullBackup, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `vens-home-backup-${getTodayDate()}.json`;
    a.click();
    showToast('📥 باکئەپ دابەزی');
  };
  ...
  const handleRestoreFile = (e) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        if (backupData.savings) saveSavings(backupData.savings);
        if (backupData.debts) saveDebts(backupData.debts);
  ...
  return (
    <>
      ...
      {toastMessage && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 bg-[#123524] text-[#D4B872] px-4 py-2.5 rounded-xl shadow-2xl font-bold text-xs border border-[#D4B872] flex items-center gap-2">
            ...
            <div className="space-y-4">
              <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-emerald-700">هەموو زانیارییەکان بە پارێزراوی لە ناوخۆی ئامێرەکەت و سەر تۆڕ پاشەکەوت دەبن.</p>
                </div>
              </div>
